...
export default function HighlightProjects({ projects }: {projects:ProjectListType}) {
  return (
    <div className="feature relative z-0 mt-20">
      {/* Background */}
      <div
        id="f-bg"
        className="bg-[url('/images/hero-bg.avif')] bg-center bg-cover w-[130%] h-[110vh] transform -translate-x-20"
      ></div>
          ...
          {projects.map((project, index) => {
            ...
            return (
              <div
                ...
                className={`sm:min-w-[400px] min-w-full bg-white flex justify-start items-start rounded-4xl p-4 text-center ${alignment}`}
              >
                <img
                  src={project.img}
                  alt={project.title}
                ...
                />
                <div className="w-full overflow-hidden mx-2 space-y-2">
                  <h3 className="text-start font-bold text-xl line-clamp-2">
                    {project.title}
                  </h3>
